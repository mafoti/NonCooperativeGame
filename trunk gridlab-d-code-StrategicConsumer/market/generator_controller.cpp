...
int generator_controller::create(void)
{
	...
	//auction_object->gameTheory.addGenerator(id);
	return 1;
}
				...
				for (index = 0; index<bid_curve_values[latency_write_section].number_bid_curve_sections; index++)
				{
					...
					if (gen_state != GEN_OFF)	//Generator is not off, so must be starting/started
					{
						...
						if (gl_object_isa(parent2, "solar", "generators"))
						{
							...
							//auction_object->gameTheory.updateSuply(id,((*complex_power).Re()) / 1000);
						...
						else
						{
							...
							//auction_object->gameTheory.updateSuply(id,((*bid_power_active) / 1000));
						}
