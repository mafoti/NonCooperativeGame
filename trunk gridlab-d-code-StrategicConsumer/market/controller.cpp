...
controller::controller(MODULE *module){
	if (oclass == NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_KEYWORD, "JACUZZI", (enumeration)SM_JACUZZI,
...
void controller::cheat(){
	...
	case SM_NONE:
		break;
	case SM_HOUSE_HEAT:
		sprintf(outdoor, "outdoor_temperature");
...
int controller::init(OBJECT *parent){
	...
	fetch(&pOutTemperature, "outdoor_temperature", parent);
	...
	//initialsetpoint0 = *pSetpoint;
	//last_p = market->init_price;
	
	/////////////////////////////////////////////////////////////////////////// game /////////////////////////////////////////////////////////////////////////////////////
	/*if (control_mode == CN_DOUBLE_RAMP){
		int id = OBJECTHDR(this)->id;
		//char namebuf[128];
...
TIMESTAMP controller::sync(TIMESTAMP t0, TIMESTAMP t1){
	...
	if (control_mode == CN_RAMP){
		...
		//printf("inside sync ramp, last_p=%f, last_q=%f, no_bid=%f, use_predictive_bidding=%d, dir=%d \n", last_p, last_q, no_bid, use_predictive_bidding, dir);
		...
		*/
		...
		if (lastmkt_id != market->market_id){
			...
			////////////////////////////////////////////////////////////////// game /////////////////////////////////////////////////////////////////
			/*workingIndicator = market->gameTheory.checkGameResults(id);
			if (!workingIndicator)
			{
			// We have to cool
			if (*pMonitor > cool_max){
			double coolingCOP = market->gameTheory.getCoolingGameResult(id);
			...
			double heatingCOP = market->gameTheory.getHeatingGameResult(id);
		...
		if (*pMonitor > cool_max){
			...
			coolingload = *pCoolingDemand;
		}
		// We have to heat
		else if (*pMonitor < heat_min){
			last_p = market->pricecap;
			last_q = *pHeatingDemand;
			heatingload = *pHeatingDemand;
		}
...
TIMESTAMP controller::postsync(TIMESTAMP t0, TIMESTAMP t1){
	...
	////////////////////////////////////////////////////////////////// game ////////////////////////////////////////////////////////////////////////
	/*if (control_mode == CN_DOUBLE_RAMP){
		//printf("controller postsync heating_demand=%f cooling_demand=%f\n", *pHeatingDemand, *pCoolingDemand);
	...
	/*double result = market->gameTheory.lossfunction(id);
	if (result > 1){
	workingIndicator = true;
	}
	else{
	...
	double prediction = market->gameTheory.getSinglePrediction();
	if (prediction > 0.18){
	workingIndicator = false;
	}
	else{
	...
	///////////////////////////////////////////////////////////////////jacuzzi////////////////////////////////////////////////////////////////
	//int id = OBJECTHDR(this)->id;
	//double* temperature = gl_get_double_by_name(parent2, "temperature");
	//market->gameTheory.updateNodeTemperature(id, *temperature);
	/*setpoint0 = 0;
	if (workingTimeslots.size() == 0){
	//workingTimeslots = market->gameTheory.getVector(id);

	std::vector<bool> defaultvector(96, false);
	defaultvector[workingRandom] = true;
	defaultvector[workingRandom-1] = true;
	defaultvector[workingRandom-2] = true;

	workingTimeslots = defaultvector;
	}
	if (workingTimeslots.size() != 0){

	bool nextAction = workingTimeslots.back();
	workingTimeslots.pop_back();
	if (nextAction){
	setpoint0 = initialsetpoint0;
	}
	...
	/*char namebuf[128];
	gl_name(OBJECTHDR(this), namebuf, 127);
	string testcase = "waterheaterController";
	if (testcase.compare(namebuf) == 0){
	printf("setpoint %f \n", setpoint0);
	char timenext_run[128];
	gl_printtime(next_run, timenext_run, 127);
