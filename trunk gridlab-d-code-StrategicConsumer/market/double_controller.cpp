...
int double_controller::init(OBJECT *parent){
	gameId = rand();
	groupId = gameId % 6;
	//printf("counter random counter=%d\n ", counter);
...
TIMESTAMP double_controller::sync(TIMESTAMP t0, TIMESTAMP t1){
	...
	if(bid_mode == BM_ON){
		...
		if (testcase.compare(namebuf2) == 0){
			if (enthousiasm > 0.142){
				printf("enthousiasm = %f, counter = %d\n", enthousiasm, counter);
				//if ((gameId % 3) == (counter % 3)){
				bid_price = 0.0;
				bid_quant = 0.0;
				//}
			}
...
TIMESTAMP double_controller::postsync(TIMESTAMP t0, TIMESTAMP t1){
	...
	if (testcase.compare(namebuf2) == 0){
		if (t0 < next_run){
			...
			TIMESTAMP newNextRun = (TIMESTAMP)(*pPeriod) + t1;
			char timenext_run[128];
			gl_printtime(newNextRun, timenext_run, 127);
			trainer.setNextAuctionTime(timenext_run);
		}
