/** $Id: jacuzzi.cpp,v 1.51 2008/02/15 00:24:14 d3j168 Exp $
Copyright (C) 2008 Battelle Memorial Institute
@file jacuzzi.cpp
@addtogroup jacuzzi Electric jacuzzi
@ingroup residential

The residential electric jacuzzi uses a hybrid thermal model that is capable
of tracking either a single-mass of water, or a dual-mass with a varying thermocline.

The driving dynamic parameters of the jacuzzi are
...
//////////////////////////////////////////////////////////////////////////
// jacuzzi CLASS FUNCTIONS
//////////////////////////////////////////////////////////////////////////
CLASS* jacuzzi::oclass = NULL;
CLASS* jacuzzi::pclass = NULL;

/**  Register the class and publish water heater object properties
**/
jacuzzi::jacuzzi(MODULE *module) : residential_enduse(module){
	// first time init
	if (oclass == NULL)
	{
		pclass = residential_enduse::oclass;
		// register the class definition
		oclass = gl_register_class(module, "jacuzzi", sizeof(jacuzzi), PC_PRETOPDOWN | PC_BOTTOMUP | PC_POSTTOPDOWN | PC_AUTOLOCK);
		if (oclass == NULL)
		...
		if (gl_publish_variable(oclass,
			...
			PT_complex, "actual_power[kVA]", PADDR(jacuzzi_actual_power), PT_DESCRIPTION, "the actual power based on the current voltage across the coils",
			PT_double, "is_jacuzzi_on", PADDR(is_jacuzzi_on), PT_DESCRIPTION, "simple logic output to determine state of jacuzzi (1-on, 0-off)",
			PT_double, "gas_fan_power[kW]", PADDR(gas_fan_power), PT_DESCRIPTION, "load of a running gas jacuzzi",
			PT_double, "gas_standby_power[kW]", PADDR(gas_standby_power), PT_DESCRIPTION, "load of a gas jacuzzi in standby",
			NULL)<1)
...
**/
int jacuzzi::init(OBJECT *parent)
{
	...
	if (parent != NULL){
		if ((parent->flags & OF_INIT) != OF_INIT){
			char objname[256];
			gl_verbose("jacuzzi::init(): deferring initialization on %s", gl_name(parent, objname, 255));
			return 2; // defer
		}
	}
	...
	if (pTair == 0){
		pTair = &sTair;
		gl_warning("jacuzzi parent lacks \'air_temperature\' property, using default");
	}
	if (pTout == 0){
		pTout = &sTout;
		gl_warning("jacuzzi parent lacks \'outside_temperature\' property, using default");
	}
	...
	if (tank_volume <= 0.0){
		...
		if (tank_volume > 100.0 || tank_volume < 20.0){
			...
			/*	TROUBLESHOOT
			All jacuzzis must be set between 40 and 100 gallons.  Most jacuzzis are assumed to be 50 gallon tanks.
			*/
		}
	}
	...
	/*	TROUBLESHOOT
	All jacuzzis must be set between 90 degF and 160 degF.
	...
	switch (shape.type){
	case MT_UNKNOWN:
		//printf(" shape.type = MT_UNKNOWN \n");
		/* normal, undriven behavior. */
		break;
	case MT_ANALOG:
		//printf(" shape.type = MT_ANALOG \n");
		if (shape.params.analog.energy == 0.0){
			GL_THROW("jacuzzi does not support fixed energy shaping");
			/*	TROUBLESHOOT
	...
	case MT_PULSED:
		...
		* pulsed loadshapes can either user time or kW values per pulse. */
		...
		break;
	case MT_MODULATED:
		//printf(" shape.type = MT_MODULATED \n");
		if (shape.params.modulated.pulsetype == MPT_TIME){
			...
			counterintuitive to the normal usage of the jacuzzi.
			*/
		}
	...
	case MT_QUEUED:
		...
		break;
	default:
		//printf(" shape.type = default \n");
		GL_THROW("jacuzzi load shape has an unknown state!");
		break;
	}
	return residential_enduse::init(parent);
}

int jacuzzi::isa(char *classname)
{
	return (strcmp(classname, "jacuzzi") == 0 || residential_enduse::isa(classname));
}


void jacuzzi::thermostat(TIMESTAMP t0, TIMESTAMP t1){
	...
	case EMPTY:
		...
		break;
	default:
		GL_THROW("jacuzzi thermostat() detected that the water heater tank is in an unknown state");
	}
...
**/
TIMESTAMP jacuzzi::presync(TIMESTAMP t0, TIMESTAMP t1){
	...
	if (Tw > 212.0){
		//GL_THROW("the jacuzzi is boiling!");
		gl_warning("jacuzzi:%i is boiling", my->id);
		/*	TROUBLESHOOT
		The temperature model for the jacuzzi has broken, or the environment around the
		jacuzzi has burst into flames.  Please post this with your model and dump files
...
**/
TIMESTAMP jacuzzi::sync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	jacuzzi_actual_power = load.power + (load.current + load.admittance * load.voltage_factor)* load.voltage_factor;
	actual_load = jacuzzi_actual_power.Re();
...
TIMESTAMP jacuzzi::postsync(TIMESTAMP t0, TIMESTAMP t1){
	return TS_NEVER;
}

TIMESTAMP jacuzzi::commit(){
	Tw_old = Tw;
...
**/
enumeration jacuzzi::tank_state(void)
{
...
**/
void jacuzzi::set_time_to_transition(void)
{
...
**/
enumeration jacuzzi::set_current_model_and_load_state(void)
{
...
void jacuzzi::update_T_and_or_h(double nHours)
{
	/*
	When this gets called (right after the jacuzzi gets sync'd),
...
double jacuzzi::actual_kW(void)
{
	...
	if (heat_needed && re_override != OV_OFF)
	{
		...
		{
			if (trip_counter++ > 10)
				GL_THROW("Water heater line voltage for jacuzzi:%d is too high, exceeds twice nominal voltage.", obj->id);
			/*	TROUBLESHOOT
			The jacuzzi is receiving twice the nominal voltage consistantly, or about 480V on what
...
inline double jacuzzi::new_time_1node(double T0, double T1)
{
	...
	//printf(" new_time_1node result = %f \n ", new_time);
	return new_time;
}

inline double jacuzzi::new_temp_1node(double T0, double delta_t)
{
	...
	return  c2 - (c2 - T0) * exp(-c1 * delta_t);	// [F]
	//result = result *0.9;
	//printf(" new_temp_1node result = %f \n ", result);
	//return result;
}


inline double jacuzzi::new_time_2zone(double h0, double h1)
{
...
inline double jacuzzi::new_h_2zone(double h0, double delta_t)
{
	if (delta_t <= ROUNDOFF)
		return h0;
...
double jacuzzi::get_Tambient(enumeration loc)
{
	double ratio;
...
void jacuzzi::wrong_model(WRONGMODEL msg)
{
	char *errtxt[] = { "model is not one-zone", "model is not two-zone" };
	OBJECT *obj = OBJECTHDR(this);
	gl_warning("%s (jacuzzi:%d): %s", obj->name ? obj->name : "(anonymous object)", obj->id, errtxt[msg]);
	throw msg; // this must be caught by the jacuzzi code, not by the core
}
...
EXPORT int create_jacuzzi(OBJECT **obj, OBJECT *parent)
{
	try
	{
		*obj = gl_create_object(jacuzzi::oclass);
		if (*obj != NULL)
		{
			jacuzzi *my = OBJECTDATA(*obj, jacuzzi);;
	...
	CREATE_CATCHALL(jacuzzi);
}

EXPORT int init_jacuzzi(OBJECT *obj)
{
	try
	{
		jacuzzi *my = OBJECTDATA(obj, jacuzzi);
	...
	INIT_CATCHALL(jacuzzi);
}

EXPORT int isa_jacuzzi(OBJECT *obj, char *classname)
{
	if (obj != 0 && classname != 0){
		return OBJECTDATA(obj, jacuzzi)->isa(classname);
	}
	else {
		return 0;
	}
...
EXPORT TIMESTAMP sync_jacuzzi(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass)
{
	try {
		jacuzzi *my = OBJECTDATA(obj, jacuzzi);
	...
	SYNC_CATCHALL(jacuzzi);
}

EXPORT TIMESTAMP commit_jacuzzi(OBJECT *obj, TIMESTAMP t1, TIMESTAMP t2)
{
	jacuzzi *my = OBJECTDATA(obj, jacuzzi);
	return my->commit();
}

EXPORT TIMESTAMP plc_jacuzzi(OBJECT *obj, TIMESTAMP t0)
{
	// this will be disabled if a PLC object is attached to the jacuzzi
	if (obj->clock <= ROUNDOFF)
